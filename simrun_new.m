...
% eval(['load ', prefix,'CN0']); hard coded

CN0 = power(10, [20.43, 21.34, 24.16, 25.48]/10);  
...
master_run_flag  = 2;  % 1=process data from scratch
...
%CN0min = 35;  % Used for the search of best fit C/N0 based upon noise SNR.
%CN0max = 45;
...
%
%%    fprintf('Skip that and use fixed CN0  ...\n')
...
if (model_check == 1)
   ...
   for kplot = testcases;
       ...
       [mp, cpdummy] = modelset( mp, 8, 0, 10000, pi/2, 1e-3, 0, ...
           [0 0 0], [0 0 0], 10000, prnhist(kplot), 'CA');
...
if (master_sim_flag == 1)
   ...
   for kplot = testcases;
       ...
       fprintf('Simulating case %5i: SV%02d at el: %2.2f deg: MSS= %8.6 \n', ...
...
for kplot = testcases
    ...
    xlabel(['Seconds past ', num2str(hourstart), 'H GPST'])
    ...
    for ksubplot=1:6
     ...
     %   plot(histbin_obs, Nobs/sum(Nobs), 'x', histbin_syn, Nsyn/sum(Nsyn), 'o');
         plot(histbin_obs, Nobs/(totalarea_obs), '-ok', ...
             histbin_syn, Nsyn/(totalarea_syn), '--k', ...
             histbin_syn-offsetmean, Nsyn/(totalarea_syn), '-k');
        if ksubplot==1
...
%for kplot=testcases
    ...
    %
    % 2-D color map of covariance matrix
    %
...
%    Psyn = cov(wfnsyn);
 ...
 %   Pobsn = Pobs./(max(max(Pobs)));
 %   Psynn = Psyn./(max(max(Psyn)));
    
 %   Pobsn = corrcoef(wfnobs);
%    Psynn = corrcoef(wfnsyn);
    
 %   subplot(4,2,subplotno)
   % contourf(delaymean_obs, delaymean_obs, 10*log10(Pobs)); dB version
 %    contourf(delaymean_obs, delaymean_obs, log(abs(Pobsn)));
 %    if subplotno == 1
%       title(strcat(subplotid(kplot), ' Measured'))   
 %       title('Measured')
 %    end
...
%     caxis([-5 0])
 ...
 %    subplot(4,2,subplotno+1)
   % contourf(delaymean_syn(1:800), delaymean_syn(1:800),
   % 10*log10(Psyn(1:800,1:800))); dB version
  %   contourf(delaymean_syn(1:800), delaymean_syn(1:800), log(abs(Psynn(1:800,1:800))))
 %    if subplotno == 1
 %       title('Simulated') 
 %    end
 %    axis('square')
