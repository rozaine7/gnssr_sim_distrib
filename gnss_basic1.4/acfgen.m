function acfstruct = acfgen( code_type, prn, FEBW, BW )
  acfstruct = struct('acf2matrix', 0, 'acfmatrix', 0, 'taumatrix', 0, ...
		     'chipsize',293,'delaystep',1);
%
% 4/2106 - modified to include the ACF for general SoOps, 
%    BW = signal bandwidth
...
elseif  code_type == 'SO'
    %
    % signals of opportunity added
    %
  %fprintf ('  Generic SoOp (BPSK assumed): \n')
  %BW = input('    Bandwidht (MHz) ?\n');
  fprintf('   Generating ACF for a %6.1f MHz digital signal.\n', BW);
  chipsize = c/(BW*1e6);           
  delaystep = chipsize/100;  % set generically - in distance units (m)
  fprintf('   Chip period:  %6.1f m \n', chipsize);
  tauref = [-150:150]' * delaystep;  
  [acfmatrix, acorr_fcn] = lambda2_function (tauref / chipsize);
   %acfmatrix = acorr_fcn_volt; dont need multiple copies
else
